...
def seleciona_usuario():
  usuarios_objetos = ContaCorrente.query.all()
...
def CriarCliente():
  ...
  try:
    cliente = ContaCorrente(numero=body["numero"], agencia=body["agencia"], saldo=body["saldo"])
    db.session.add(cliente)
    db.session.commit()

    return func_response(200, "usuario", cliente.to_json(), "criado com sucesso!")
  except Exception as e:
    return func_response(400, "usuario", {}, "Erro ao cadastrar")
@app.route("/cliente/<id>", methods=["DELETE"])
def DeletarCliente(id):
  cliente = ContaCorrente.query.filter_by(id=id).first()

  try:
    db.session.delete(cliente)
    db.session.commit()

    return func_response(200, "usuario", cliente.to_json(), "deletado com sucesso!")
  except Exception as e:
    return func_response(400, "usuario", {}, "Erro ao cadastrar")
app.run()